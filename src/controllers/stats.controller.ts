...
export const getPageViewsStats = async (req: Request, res: Response) => {
    try {
        const statsQuery = (req as StatsRequest).statsQuery;
        const results = await getPageViewsStatsService(statsQuery);
        console.log('> Results:', results);
        res.status(StatusCodes.OK).json({ page: statsQuery.page, limit: statsQuery.limit, total: results.length, data: results });
    } catch (err) {
...
export const getUserActivityStats = async (req: Request, res: Response) => { 
    try {
        ...
        const data = await getUserActivityStatsService(userId, statsQuery);
        res.status(StatusCodes.OK).json({ page: statsQuery.page, limit: statsQuery.limit, total: data.length, data });
    } catch (err) {
